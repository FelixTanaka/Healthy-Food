import Sidebar from "../components/Sidebar";
import { useState } from "react";

export default function Selller() {
    const [showModal, setShowModal] = useState(false);

    return (
        <div className="flex bg-gray-100 min-h-screen">
            <Sidebar />

            <div className="flex-1 p-6">
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-2xl font-semibold">Seller</h2>

                    <button className="bg-orange-500 hover:bg-orange-600 text-white px-4 py-2 rounded-lg">
                        + Tambah Seller
                    </button>
                </div>

                <div className="mb-4">
                    <input
                        type="text"
                        placeholder="Cari seller..."
                        className="w-full md:w-1/3 bg-white border border-gray-300 rounded-lg px-4 py-2 shadow-sm focus:outline-none focus:ring-2 focus:ring-orange-200 focus:border-orange-200"
                    />
                </div>

                <div className="bg-white rounded-xl shadow overflow-hidden">
                    <table className="w-full text-sm">
                        <thead className="bg-orange-100 text-orange-600 uppercase text-xs">
                        <tr>
                            <th className="p-3 text-center">No</th>
                            <th className="p-3 text-left">Nama</th>
                            <th className="p-3 text-left">Email</th>
                            <th className="p-3 text-left">No Telp</th>
                            <th className="p-3 text-left">Nama Toko</th>
                            <th className="p-3 text-center">Aksi</th>
                        </tr>
                        </thead>

                        <tbody className="text-gray-700">
                        
                            <tr className="border-t hover:bg-gray-50 transition">
                                <td className="p-3 text-center">1</td>
                                <td className="p-3 font-medium">Budi</td>
                                <td className="p-3">budi@gmail.com</td>
                                <td className="p-3">08123456789</td>
                                <td className="p-3">Warung Budi</td>
                                <td className="p-3">
                                <div className="flex justify-center gap-2">
                                    <button className="px-3 py-1 text-xs bg-gray-500 hover:bg-gray-600 text-white rounded">
                                        Detail
                                    </button>
                                    <button className="px-3 py-1 text-xs bg-blue-500 hover:bg-blue-600 text-white rounded">
                                        Edit
                                    </button>
                                    <button className="px-3 py-1 text-xs bg-red-500 hover:bg-red-600 text-white rounded">
                                        Hapus
                                    </button>
                                </div>
                                </td>
                            </tr>

                            <tr className="border-t hover:bg-gray-50 transition">
                                <td className="p-3 text-center">2</td>
                                <td className="p-3 font-medium">Siti</td>
                                <td className="p-3">siti@gmail.com</td>
                                <td className="p-3">08234567891</td>
                                <td className="p-3">Toko Siti</td>
                                <td className="p-3">
                                <div className="flex justify-center gap-2">
                                    <button className="px-3 py-1 text-xs bg-gray-500 hover:bg-gray-600 text-white rounded" onClick={() => setShowModal(true)}>
                                        Detail
                                    </button>
                                    <button className="px-3 py-1 text-xs bg-blue-500 hover:bg-blue-600 text-white rounded">
                                        Edit
                                    </button>
                                    <button className="px-3 py-1 text-xs bg-red-500 hover:bg-red-600 text-white rounded">
                                        Hapus
                                    </button>
                                </div>
                                </td>
                            </tr>

                        </tbody>
                    </table>
                </div>
            </div>

            {showModal && (
                <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50">
                    
                    <div className="bg-white w-full max-w-xl rounded-2xl shadow-xl p-6 relative animate-fadeIn">

                        <div className="-mx-6 px-6 flex justify-between items-center border-b pb-3 mb-4">
                            <h2 className="text-lg font-semibold text-gray-800">
                                Detail Seller
                            </h2>

                            <button
                                onClick={() => setShowModal(false)}
                                className="text-gray-400 hover:text-gray-700 text-lg"
                            >
                                ✕
                            </button>
                        </div>

                        <div className="grid grid-cols-2 gap-4 text-sm text-gray-700">

                            <div>
                                <p className="text-gray-500">Nama</p>
                                <p className="font-medium">Budi</p>
                            </div>

                            <div>
                                <p className="text-gray-500">Email</p>
                                <p className="font-medium">budi@gmail.com</p>
                            </div>

                            <div>
                                <p className="text-gray-500">No Telp</p>
                                <p className="font-medium">08123456789</p>
                            </div>

                            <div>
                                <p className="text-gray-500">Nama Toko</p>
                                <p className="font-medium">Warung Budi</p>
                            </div>

                            <div className="col-span-2">
                                <p className="text-gray-500">Deskripsi</p>
                                <p className="font-medium">
                                    Menjual makanan sehat dan bergizi setiap hari
                                </p>
                            </div>

                            <div className="col-span-2">
                                <p className="text-gray-500">Alamat</p>
                                <p className="font-medium">Yogyakarta</p>
                            </div>

                            <div>
                                <p className="text-gray-500">Jam Buka</p>
                                <p className="font-medium">08:00</p>
                            </div>

                            <div>
                                <p className="text-gray-500">Jam Tutup</p>
                                <p className="font-medium">20:00</p>
                            </div>

                        </div>

                        <div className="mt-6 flex justify-end">
                            <button
                                onClick={() => setShowModal(false)}
                                className="px-4 py-2 bg-gray-100 hover:bg-gray-200 rounded-lg text-sm"
                            >
                                Tutup
                            </button>
                        </div>

                    </div>
                </div>
            )}
        </div>
    );
}